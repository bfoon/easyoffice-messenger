...
class ApiService {
  ...
  /// Upload a picture or file to a room.
  ///
  /// Hits the server's rooms/<room_id>/upload/ endpoint with a multipart body.
  /// The server creates the ChatMessage and broadcasts it; the polling loop in
  /// the chat screen will then display it.
  ...
  // This is a helper.
  Future<http.Response?> httpGetAbsolute(
    String absoluteUrl,
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('httpGetAbsolute failed: $e');
      }
...
/// Files → Messages: attach an existing Files-app file to a chat by id.
  ...
  Future<List<RemoteFile>> files({
    ...
    if (folder.isNotEmpty) {
      params.add('folder=${Uri.encodeQueryComponent(folder)}');
    }
  ...
  /// Upload a phone file into the Files app.
  ///
  /// Returns the new file id, or null if upload failed.
  Future<String?> uploadToFiles(
    String filePath, {
    String? folderId,
  }) async {
    final uri = AppConfig.api('files/upload/');
    final req = http.MultipartRequest('POST', uri);
    ...
    if (streamed.statusCode == 401 && _refresh != null) {
      if (await refreshAccess()) {
        return uploadToFiles(filePath, folderId: folderId);
      }
    }
  ...
  Future<List<SignRequest>> signRequests({bool openOnly = true}) async {
    final res = await _send(
      'GET',
      'sign/requests/?status=${openOnly ? 'open' : 'all'}',
    );
  ...
  Future<bool> signFillField(
    ...
    final res = await _send(
      'POST',
      'sign/requests/$requestId/fields/$fieldId/',
      body: {'value': value},
    );
  ...
  Future<String?> signSubmit(
    ...
    final res = await _send(
      'POST',
      'sign/requests/$requestId/submit/',
      body: {
  ...
  Future<String?> signDecline(String requestId, String reason) async {
    final res = await _send(
      'POST',
      'sign/requests/$requestId/decline/',
      body: {'reason': reason},
    );
  ...
  /// The signing PDF needs the Authorization header.
  ///
  ...
  ///
  /// Others still see it. Returns true on success.
